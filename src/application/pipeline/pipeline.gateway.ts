...
export class PipelineGateway {
    ...
    async handleExecutePipeline(@MessageBody() createPipelineDto: CreatePipelineDto): Promise<void> {
        try {
            const result = await this.pipelineService.executePipeline(createPipelineDto, (update) => {
